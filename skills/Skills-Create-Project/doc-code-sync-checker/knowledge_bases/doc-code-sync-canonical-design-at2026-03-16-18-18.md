...
- 현재 `scripts/doc_code_sync.py`는 첫 두 vertical slice가 구현된 상태다.
- 구현된 pair / rule kind:
  - `agent-task-packet/references/packet-fields.md` ↔ `agent-task-packet/scripts/packet_builder.py`
  - `required_field`
  - `codex-worktree-dispatch/references/dispatch-fields.md` ↔ `codex-worktree-dispatch/scripts/dispatch_manager.py`
  - `path_safety`
  - `codex-worktree-dispatch/references/dispatch-fields.md` ↔ `codex-worktree-dispatch/scripts/dispatch_manager.py`
  - `transition_rule`
  - `codex-worktree-dispatch/references/dispatch-fields.md` ↔ `codex-worktree-dispatch/scripts/dispatch_manager.py`
...
- 이미 구현된 것:
  ...
  - `required_field` 공통 rule schema와 smoke report
  - `path_safety` 공통 rule schema와 smoke report
  - `transition_rule` 공통 rule schema와 smoke report
  - `enum_value` 공통 rule schema와 smoke report
- 현재 real pair smoke 결과:
  - `required_field`: drift 0건
  - `path_safety`: drift 0건
  - `transition_rule`: drift 0건
  - `enum_value`: drift 0건
- 다음 구현 후보:
  - `mismatch`의 semantic/value-level 판정 강화
