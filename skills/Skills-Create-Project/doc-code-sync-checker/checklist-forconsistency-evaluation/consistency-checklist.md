...
> source of truth: `knowledge_bases/doc-code-sync-canonical-design-at2026-03-16-18-18.md`
...
- [ ] **B-02**: code는 repo-wide 범용 엔진이 아니라 명시적으로 지정한 문서/스크립트 쌍을 검사하는 pairwise checker 경계를 유지한다
...
- [ ] **D-01**: code가 문서와 코드 양쪽에서 공통으로 비교 가능한 규칙(rule) object를 기본 데이터 모델로 사용한다
...
- [ ] **D-05**: 우선 지원 규칙 유형이 `필수 필드 / enum·상수 / 상태 전이표 / 경로 규칙`으로 명시돼 있다
- [ ] **D-06**: enum/상수 비교에서는 문서의 허용값 집합과 코드의 허용값 집합을 같은 rule set으로 정규화할 수 있다
- [ ] **D-07**: enum drift는 `doc-only value`와 `code-only value`를 분리해서 판정할 수 있다
- [ ] **D-08**: `status enum` 비교와 `transition_rule` 비교가 서로 다른 계약이라는 점이 구분돼 있다
...
- [ ] **F-02**: 입력 범위는 기본적으로 `reference 문서 1개 + script 1개`의 명시적 쌍이다
...
- [ ] **G-03**: scaffold 단계와 실제 구현 단계, 그리고 "가능한 미래 기능"과 "현재 동작"이 혼동되지 않는다
