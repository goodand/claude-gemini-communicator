...
Usage:
    ...
    python3 dependency_slice_planner.py emit-wrapper-path-mutation-register-contract
    python3 dependency_slice_planner.py build-wrapper-path-mutation-register --input-wrapper-path-mutation-manifest <file>
    python3 dependency_slice_planner.py validate-wrapper-path-mutation-register --input-wrapper-path-mutation-register <file>
...
WRAPPER_PATH_MUTATION_MANIFEST_TOP_LEVEL_REQUIRED = ["manifest_count", "manifests"]
WRAPPER_PATH_MUTATION_MANIFEST_REQUIRED_FIELDS = [
    "root_path",
    "wrapper_paths",
    "path_mutations",
    "reason",
]
WRAPPER_PATH_MUTATION_REGISTER_TOP_LEVEL_REQUIRED = [
    "status",
    "generated_at",
    "algorithm_family",
    "version",
...
WRAPPER_PATH_MUTATION_REGISTER_REQUIRED_FIELDS = [
    "root_path",
    "wrapper_paths",
    "path_mutations",
    "reason",
]
WRAPPER_PATH_MUTATION_REGISTER_MAX_WRAPPER_PATHS = 8
WRAPPER_PATH_MUTATION_REGISTER_MAX_PATH_MUTATIONS = 8
WRAPPER_PATH_MUTATION_REGISTER_SIGNAL = "wrapper_path_mutation_register"
...
def emit_wrapper_path_mutation_register_contract() -> dict[str, object]:
    return {
        "status": "ok",
        "generated_at": _now_iso(),
        "contract_family": WRAPPER_PATH_MUTATION_REGISTER_SIGNAL,
        "version": "v0.1.0",
        "required_top_level_fields": WRAPPER_PATH_MUTATION_REGISTER_TOP_LEVEL_REQUIRED,
        "register_required_fields": WRAPPER_PATH_MUTATION_REGISTER_REQUIRED_FIELDS,
        "manifest_required_fields": WRAPPER_PATH_MUTATION_MANIFEST_REQUIRED_FIELDS,
        "top_level_schema": {
            "status": "enum(ok|invalid_inputs)",
            "generated_at": "str",
            "algorithm_family": "str",
        ...
        "register_field_schema": {
            "root_path": "str",
            "wrapper_paths": f"list[str] (<= {WRAPPER_PATH_MUTATION_REGISTER_MAX_WRAPPER_PATHS} after bound)",
            "path_mutations": f"list[str] (<= {WRAPPER_PATH_MUTATION_REGISTER_MAX_PATH_MUTATIONS} after bound)",
            "reason": "str",
        },
        "manifest_field_schema": {
            "root_path": "str",
        ...
        "bounding_rules": {
            "max_wrapper_paths_per_register": WRAPPER_PATH_MUTATION_REGISTER_MAX_WRAPPER_PATHS,
            "max_path_mutations_per_register": WRAPPER_PATH_MUTATION_REGISTER_MAX_PATH_MUTATIONS,
        },
    }
...
def build_wrapper_path_mutation_register(
    input_manifest_path: Path,
    input_manifest_payload: dict[str, object],
) -> tuple[dict[str, object], int]:
    errors: list[str] = []

    for field in WRAPPER_PATH_MUTATION_MANIFEST_TOP_LEVEL_REQUIRED:
        if field not in input_manifest_payload:
            errors.append(f"missing top-level field: {field}")

    manifest_count = input_manifest_payload.get("manifest_count")
    if not isinstance(manifest_count, int) or manifest_count < 0:
        errors.append("manifest_count must be non-negative int")

    raw_manifests = input_manifest_payload.get("manifests")
    if not isinstance(raw_manifests, list):
        errors.append("manifests must be list[dict]")
        raw_manifests = []

    normalized_manifests: list[dict[str, object]] = []
    for index, raw_manifest in enumerate(raw_manifests, start=1):
        prefix = f"manifests[{index}]."
        if not isinstance(raw_manifest, dict):
            errors.append(f"{prefix[:-1]} must be dict")
            continue
        for field in WRAPPER_PATH_MUTATION_MANIFEST_REQUIRED_FIELDS:
            if field not in raw_manifest:
                errors.append(f"missing field: {prefix}{field}")
        root_path = raw_manifest.get("root_path")
        if "root_path" in raw_manifest and (not isinstance(root_path, str) or not root_path.strip()):
            errors.append(f"{prefix}root_path must be non-empty str")
        _validate_list_of_strings(raw_manifest.get("wrapper_paths"), "wrapper_paths", errors, prefix)
        _validate_list_of_strings(raw_manifest.get("path_mutations"), "path_mutations", errors, prefix)
        reason = raw_manifest.get("reason")
        if "reason" in raw_manifest and (not isinstance(reason, str) or not reason.strip()):
            errors.append(f"{prefix}reason must be non-empty str")
        if (
            isinstance(raw_manifest.get("wrapper_paths"), list)
            and isinstance(raw_manifest.get("path_mutations"), list)
            and not raw_manifest["wrapper_paths"]
            and not raw_manifest["path_mutations"]
        ):
            errors.append(f"{prefix[:-1]} must include at least one wrapper_path or path_mutation")
        normalized_manifests.append(raw_manifest)

    if isinstance(manifest_count, int) and isinstance(raw_manifests, list) and manifest_count != len(raw_manifests):
        errors.append("manifest_count must match len(manifests)")
        ...
        return (
            {
                "status": "invalid_inputs",
                "generated_at": _now_iso(),
                "algorithm_family": WRAPPER_PATH_MUTATION_REGISTER_SIGNAL,
                "version": "v0.1.0",
                "input_artifacts": {
                    "wrapper_path_mutation_manifest": _relative_or_str(input_manifest_path),
                },
                "register_count": 0,
                "registers": [],
                "invalid_inputs": ["wrapper_path_mutation_manifest"],
                "error_count": len(errors),
                "errors": errors,
    ...
    root_order: list[str] = []
    for raw_manifest in normalized_manifests:
        root_path = str(raw_manifest.get("root_path", ""))
        if root_path not in grouped:
            grouped[root_path] = {
                "wrapper_paths": [],
                "path_mutations": [],
                "manifest_count": 0,
            }
            root_order.append(root_path)
        grouped[root_path]["wrapper_paths"].extend(
            [value for value in raw_manifest.get("wrapper_paths", []) if isinstance(value, str)]
        )
        grouped[root_path]["path_mutations"].extend(
            [value for value in raw_manifest.get("path_mutations", []) if isinstance(value, str)]
        )
        grouped[root_path]["manifest_count"] = int(grouped[root_path]["manifest_count"]) + 1

    registers: list[dict[str, object]] = []
    for root_path in root_order:
        group = grouped[root_path]
        wrapper_paths = _dedupe_preserve_order(group["wrapper_paths"])
        path_mutations = _dedupe_preserve_order(group["path_mutations"])
        wrapper_paths = wrapper_paths[:WRAPPER_PATH_MUTATION_REGISTER_MAX_WRAPPER_PATHS]
        path_mutations = path_mutations[:WRAPPER_PATH_MUTATION_REGISTER_MAX_PATH_MUTATIONS]
        reason_parts = [
            f"manifest_count={group['manifest_count']}",
            f"wrapper_path_count={len(wrapper_paths)}",
            f"path_mutation_count={len(path_mutations)}",
        ]
        if len(group["wrapper_paths"]) > len(wrapper_paths):
            reason_parts.append(f"wrapper_paths_bounded_to={WRAPPER_PATH_MUTATION_REGISTER_MAX_WRAPPER_PATHS}")
        if len(group["path_mutations"]) > len(path_mutations):
            reason_parts.append(f"path_mutations_bounded_to={WRAPPER_PATH_MUTATION_REGISTER_MAX_PATH_MUTATIONS}")
        registers.append(
            {
                "root_path": root_path,
...
def validate_wrapper_path_mutation_register(
    input_path: Path,
    payload: dict[str, object],
) -> dict[str, object]:
    errors: list[str] = []

    for field in WRAPPER_PATH_MUTATION_REGISTER_TOP_LEVEL_REQUIRED:
        if field not in payload:
            errors.append(f"missing top-level field: {field}")
    ...
    input_manifest = payload.get("input_artifacts", {})
    if not isinstance(input_manifest, dict):
        errors.append("input_artifacts must be dict[str,str]")
    elif not isinstance(input_manifest.get("wrapper_path_mutation_manifest"), str) or not input_manifest.get(
        "wrapper_path_mutation_manifest", ""
    ).strip():
        errors.append("input_artifacts.wrapper_path_mutation_manifest must be non-empty str")
    ...
    for index, raw_register in enumerate(raw_registers, start=1):
        ...
        if not isinstance(raw_register, dict):
            errors.append(f"{prefix[:-1]} must be dict")
            continue
        for field in WRAPPER_PATH_MUTATION_REGISTER_REQUIRED_FIELDS:
            if field not in raw_register:
                errors.append(f"missing field: {prefix}{field}")
        root_path = raw_register.get("root_path")
        if "root_path" in raw_register and (not isinstance(root_path, str) or not root_path.strip()):
        ...
        if "reason" in raw_register and (not isinstance(raw_register.get("reason"), str) or not raw_register.get("reason", "").strip()):
            errors.append(f"{prefix}reason must be non-empty str")
        _validate_list_of_strings(raw_register.get("wrapper_paths"), "wrapper_paths", errors, prefix)
        _validate_list_of_strings(raw_register.get("path_mutations"), "path_mutations", errors, prefix)
        if isinstance(raw_register.get("wrapper_paths"), list) and len(raw_register["wrapper_paths"]) > WRAPPER_PATH_MUTATION_REGISTER_MAX_WRAPPER_PATHS:
            errors.append(f"{prefix}wrapper_paths must contain <= {WRAPPER_PATH_MUTATION_REGISTER_MAX_WRAPPER_PATHS} items")
        if isinstance(raw_register.get("path_mutations"), list) and len(raw_register["path_mutations"]) > WRAPPER_PATH_MUTATION_REGISTER_MAX_PATH_MUTATIONS:
            errors.append(f"{prefix}path_mutations must contain <= {WRAPPER_PATH_MUTATION_REGISTER_MAX_PATH_MUTATIONS} items")
        if (
            isinstance(raw_register.get("wrapper_paths"), list)
            and isinstance(raw_register.get("path_mutations"), list)
            and not raw_register["wrapper_paths"]
            and not raw_register["path_mutations"]
        ):
            errors.append(f"{prefix[:-1]} must include at least one wrapper_path or path_mutation")
    ...
    return {
        "status": "valid" if not errors else "invalid",
        "generated_at": _now_iso(),
        "contract_family": "wrapper_path_mutation_register_validation",
        "input_wrapper_path_mutation_register": _relative_or_str(input_path),
        "error_count": len(errors),
        "errors": errors,
    }
...
def render_contract_markdown(payload: dict[str, object]) -> str:
    ...
    if payload["contract_family"] == "handoff_packet_contract":
        ...
        required_fields = payload["required_top_level_fields"]
        section_title = "Required Top-Level Fields"
    elif payload["contract_family"] == "wrapper_path_mutation_register":
        title = "wrapper_path_mutation_register contract"
        required_fields = payload["required_top_level_fields"]
        section_title = "Required Top-Level Fields"
    ...
    if payload["contract_family"] == "slice_manifest_contract":
        ...
        for field in payload["register_required_fields"]:
            lines.append(f"- `{field}`")
        lines.extend(["", "## Manifest Required Fields", ""])
        for field in payload["manifest_required_fields"]:
            lines.append(f"- `{field}`")
...
def render_validation_markdown(payload: dict[str, object]) -> str:
    ...
    elif payload["contract_family"] == "wrapper_path_mutation_register_validation":
        title = "wrapper_path_mutation_register validation"
        input_label = "input_wrapper_path_mutation_register"
...
def cmd_emit_wrapper_path_mutation_register_contract(args: argparse.Namespace) -> int:
    payload = emit_wrapper_path_mutation_register_contract()
    if args.output_json:
        output_json = Path(args.output_json)
        output_json.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
        print(f"[INFO] JSON artifact written: {output_json}", file=sys.stderr)
...
def cmd_validate_wrapper_path_mutation_register(args: argparse.Namespace) -> int:
    input_path = Path(args.input_wrapper_path_mutation_register)
    payload = _load_json(input_path)
    validation = validate_wrapper_path_mutation_register(input_path, payload)
    if args.output_json:
        output_json = Path(args.output_json)
        output_json.write_text(json.dumps(validation, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
        print(f"[INFO] JSON artifact written: {output_json}", file=sys.stderr)
...
def cmd_build_wrapper_path_mutation_register(args: argparse.Namespace) -> int:
    input_manifest_path = Path(args.input_wrapper_path_mutation_manifest)
    input_manifest_payload = _load_json(input_manifest_path)
    report, exit_code = build_wrapper_path_mutation_register(
        input_manifest_path,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    emit_wrapper_path_mutation_register_parser = subparsers.add_parser(
        "emit-wrapper-path-mutation-register-contract",
        help="Emit the canonical contract for wrapper_path_mutation_register.json.",
    )
    emit_wrapper_path_mutation_register_parser.add_argument("--output-json", help="Optional output path for contract JSON.")
    emit_wrapper_path_mutation_register_parser.add_argument("--output-md", help="Optional output path for contract markdown summary.")
    emit_wrapper_path_mutation_register_parser.set_defaults(func=cmd_emit_wrapper_path_mutation_register_contract)
    ...
    validate_wrapper_path_mutation_register_parser = subparsers.add_parser(
        "validate-wrapper-path-mutation-register",
        help="Validate a wrapper_path_mutation_register artifact against the canonical contract.",
    )
    validate_wrapper_path_mutation_register_parser.add_argument(
        "--input-wrapper-path-mutation-register",
        required=True,
        help="Path to wrapper_path_mutation_register JSON.",
    )
    validate_wrapper_path_mutation_register_parser.add_argument("--output-json", help="Optional output path for validation JSON.")
    validate_wrapper_path_mutation_register_parser.add_argument("--output-md", help="Optional output path for validation markdown summary.")
    validate_wrapper_path_mutation_register_parser.set_defaults(func=cmd_validate_wrapper_path_mutation_register)
    ...
    build_wrapper_path_mutation_register_parser = subparsers.add_parser(
        "build-wrapper-path-mutation-register",
        help="Build wrapper_path_mutation_register artifacts from a normalized wrapper/path-mutation manifest.",
    )
    build_wrapper_path_mutation_register_parser.add_argument(
        "--input-wrapper-path-mutation-manifest",
        required=True,
        help="Path to wrapper_path_mutation_manifest JSON.",
    )
    build_wrapper_path_mutation_register_parser.add_argument("--output-json", help="Optional output path for register JSON.")
    build_wrapper_path_mutation_register_parser.add_argument("--output-md", help="Optional output path for register markdown summary.")
    build_wrapper_path_mutation_register_parser.set_defaults(func=cmd_build_wrapper_path_mutation_register)
