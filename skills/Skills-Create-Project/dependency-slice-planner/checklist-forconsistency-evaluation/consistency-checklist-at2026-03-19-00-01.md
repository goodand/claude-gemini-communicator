# dependency-slice-planner 정합성 평가 체크리스트

> 목적: `dependency-slice-planner`가 coarse tree split이 아니라 dependency-aware slice decision + handoff artifact producer 역할로 고정되는지 점검한다.
> source of truth: `knowledge_bases/dependency-slice-planner-algorithm-kb-at2026-03-18-23-02.md`의 `Canonical Design Takeaways`, `Canonical Input Signals`, `Recommended Planner Algorithm`, `Canonical Output Contract`

## A. Identity

- [ ] 이 skill의 핵심 목적이 `repository structure + dependency evidence -> parallel-safe slices + handoff artifacts`로 고정돼 있다
- [ ] 이 skill이 단순 디렉토리 분할기가 아니라 `tree-based seed + dependency-aware refinement` planner로 정의돼 있다
...
- [ ] final fan-out launch는 planner의 직접 책임이 아님이 명시돼 있다
...
- [ ] KB만 읽어도 planner input/output contract가 닫혀 있어야 한다는 self-contained rule이 유지된다
...
- [ ] planner 단계가 `inventory -> coarse slice seed -> static dependency overlay -> refinement -> runtime overlay -> stop rules -> final slice proposal` 순서로 고정돼 있다
...
- [ ] 외부에서 가져와도 되는 계층이 `parser/extractor/syntax inventory/graph rule engine`으로 제한돼 있다
- [ ] 내부에 남겨야 하는 계층이 `normalized graph schema`, `wrapper/path-mutation collector`, `refinement logic`, `stop rules`, `slice manifest generation`, `handoff artifact generation`으로 고정돼 있다
