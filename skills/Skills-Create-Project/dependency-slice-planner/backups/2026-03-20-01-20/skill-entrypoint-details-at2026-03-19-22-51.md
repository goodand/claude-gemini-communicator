...
- [scripts/dependency_slice_planner.py](../scripts/dependency_slice_planner.py) — `slice_manifest`, `handoff_packet`, `inventory_snapshot`, `slice_seed_candidates`, `static_dependency_overlay`, `runtime_overlay`, `unobserved_path_register`, `inventory_path_index` contract emit/validate + `seed_to_refinement_report`, `stop_rule_evaluator`, `final_slice_proposal_generator`
