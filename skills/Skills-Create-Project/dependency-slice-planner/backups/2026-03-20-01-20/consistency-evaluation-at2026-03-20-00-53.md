...
- assessment:
  ...
  - 하지만 KB가 strongly recommended로 분리한 `wrapper/path-mutation register`, `shared hub summary`, `cycle/diamond/phantom anomaly ledger`는 독립 artifact로 닫히지 않았다.
- follow_up:
  - richer signal artifact를 별도 contract로 분리하거나 upstream extractor bridge를 더 명시할 것
