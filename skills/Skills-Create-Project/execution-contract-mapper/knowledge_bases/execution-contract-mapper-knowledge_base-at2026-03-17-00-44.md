...
- updated_at: `2026-03-17` (v0.1.1: promoted to hybrid_kb with canonical design takeaways)
- canonical_role: `execution-contract-mapper를 위한 hybrid_kb`
...
| `execution-contract-mapper-knowledge_base-at2026-03-17-00-44.md` (이 파일) | execution contract 조사 자산과 canonical slice를 함께 담는 hybrid_kb |
...
- 이 문서는 `execution-contract-mapper`의 첫 `hybrid_kb`다.
...
- 이 skill의 핵심 목적은 `개념 공간 -> 실행 계약 공간` 변환이다.
- `execution-contract-mapper`는 checklist, schema, CLI surface, rule schema처럼 agent가 직접 따를 수 있는 contract artifact를 만드는 중간층이다.
...
- consistency checklist는 implementation checklist보다 먼저 고정되며, implementation checklist는 그 계약을 작업 단위로 내린다.
- v0.1의 우선 contract family는 `rule_schema`, `schema_contract`, `cli_contract`다.
- `contract_diff_basis`는 v0.1의 후속 층이며, 먼저 stable contract artifact를 만든 뒤 붙인다.
- machine-readable contract artifact와 human-readable contract summary를 분리해서 남기는 편이 맞다.
- 최소 contract unit은 `kind`, `name`, `source`, `value`, `evidence`를 가져야 한다.
- rule schema는 checklist item을 codebase와 compare 가능한 구조적 object로 내려주는 층이다.
- schema contract는 field, type, required, constraint를 구조화해 validator/diff tool이 읽을 artifact로 고정하는 층이다.
- CLI contract는 subcommand, option, argument, help, exit behavior를 안정적으로 추출 가능한 surface로 본다.
- runtime evidence 수집과 실행 결과 해석은 `evidence-trace-auditor`가 맡는 후행 책임이다.
- concept-space로의 역방향 lifting은 `contract-to-concept-mapper`가 맡는 인접 책임이다.

## Current Implementation Target

- 현재 선택한 KB profile은 `hybrid_kb`다.
- 이유는 공식 spec/doc 조사 자산을 유지하면서도, 바로 다음 단계인 consistency checklist의 source of truth가 필요하기 때문이다.
- 다음 단계는 이 문서의 `Canonical Design Takeaways`를 기준으로 정합성 평가용 checklist를 만드는 것이다.
- v0.1의 첫 vertical slice는 `rule_schema`가 가장 자연스럽다.
- 그 다음 slice 후보는 `schema_contract`, `cli_contract`, `contract_diff_basis` 순서다.
