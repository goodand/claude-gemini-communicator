...
class EvidenceToKnowledgePromoterTest(unittest.TestCase):
    ...
    def test_help(self) -> None:
        ...
        self.assertIn("apply-canonical-kb-patch", result.stdout)
    ...
    def test_apply_hybrid_kb_patch_hold_only_copies(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir)
            patch_plan = self._write_json(base, "patch.json", {
                "status": "ok",
            ...
            target_kb = base / "target.md"
            target_kb.write_text("# KB\n\n## Canonical Design Takeaways\n\n- existing\n", encoding="utf-8")
            output_kb = base / "output.md"
            result = subprocess.run(
                [
                    "python3",
                    str(SCRIPT),
            ...
            self.assertEqual(output_kb.read_text(encoding="utf-8"), target_kb.read_text(encoding="utf-8"))

    def test_apply_hybrid_kb_patch_promote(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir)
            patch_plan = self._write_json(base, "patch.json", {
                "status": "ok",
            ...
            text = output_kb.read_text(encoding="utf-8")
            self.assertIn("promoted lesson", text)
            self.assertIn("promoted delta", text)

    def test_apply_canonical_kb_patch_hold_only_copies(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir)
            patch_plan = self._write_json(base, "canonical-patch.json", {
                "status": "ok",
                ...
                "planned_operations": [
                    {
                        ...
                        "reason": "hold case",
                        "evidence": "lesson_candidate_present",
                    }
                ],
            })
            target_kb = base / "canonical.md"
            target_kb.write_text("# Canonical\n\n## Canonical Design Takeaways\n\n- existing\n", encoding="utf-8")
            output_kb = base / "canonical-output.md"
            result = subprocess.run(
                [
                    "python3",
                    str(SCRIPT),
            ...
            self.assertEqual(result.returncode, 0, msg=result.stderr)
            payload = json.loads(result.stdout)
            self.assertEqual(payload["contract_family"], "canonical_kb_patch_apply_result")
            self.assertEqual(payload["applied_count"], 0)
            self.assertEqual(payload["skipped_count"], 1)
            self.assertEqual(output_kb.read_text(encoding="utf-8"), target_kb.read_text(encoding="utf-8"))

    def test_apply_canonical_kb_patch_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir)
            patch_plan = self._write_json(base, "canonical-patch.json", {
                "status": "ok",
                "contract_family": "canonical_kb_patch_plan",
                "patch_decision": "candidate",
                "planned_operations": [
                    {
                        "op": "append",
                        "target_section": "Canonical Design Takeaways",
