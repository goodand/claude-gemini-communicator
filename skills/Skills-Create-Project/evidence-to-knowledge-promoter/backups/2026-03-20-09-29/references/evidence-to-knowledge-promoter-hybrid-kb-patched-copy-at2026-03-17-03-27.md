...
- promoted lesson: `verified-evidence-backed-fix-pattern` — 재사용 가능한 lesson candidate를 canonical takeaway로 올린다.
...
- promoted delta: `typed_mismatch_count` — 수치 변화가 닫힌 delta를 implementation target evidence로 기록한다.

- promoted delta: `total_finding_count` — 수치 변화가 닫힌 delta를 implementation target evidence로 기록한다.
## Research Focus
...
- promoted finding: `status:ready` — 반복 해석 전 단계의 verified finding을 supporting note로 남긴다.

- promoted finding: `status:running` — 반복 해석 전 단계의 verified finding을 supporting note로 남긴다.

- promoted finding: `status` — 반복 해석 전 단계의 verified finding을 supporting note로 남긴다.
