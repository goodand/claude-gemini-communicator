...
class SliceExperimentLabTest(unittest.TestCase):
    def test_help_runs(self) -> None:
        ...
        self.assertIn("bridge-quick-validate-artifact", result.stdout)
        self.assertIn("bridge-captured-smoke-to-bundle", result.stdout)
    ...
    def test_evaluate_bundle_ready_for_next_slice(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
                ...
                json.dumps(
                    {
                        ...
                        "next_slice_candidate": "static_dependency_overlay_contract",
                    }
                ),
                encoding="utf-8",
            )
            ...
            self.assertEqual(payload["recommended_next_slice"], "static_dependency_overlay_contract")
    ...
    def test_evaluate_bundle_hold_when_quick_validate_failed(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIsNone(payload["recommended_next_slice"])
    ...
    def test_bridge_quick_validate_artifact_and_evaluate_bundle(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            contract = tmp / "contract.json"
            valid = tmp / "valid.json"
            invalid = tmp / "invalid.json"
            quick_validate_artifact = tmp / "quick-validate.json"
            bundle_draft = tmp / "bundle-draft.json"
            bridge_json = tmp / "bridge.json"
            bridge_md = tmp / "bridge.md"
            normalized_bundle = tmp / "bundle.json"
            contract.write_text(json.dumps(CONTRACT_ARTIFACT), encoding="utf-8")
            valid.write_text(json.dumps(VALID_ARTIFACT), encoding="utf-8")
            invalid.write_text(json.dumps(INVALID_ARTIFACT), encoding="utf-8")
            quick_validate_artifact.write_text(
                json.dumps(
                    {
                        "status": "passed",
                ...
                json.dumps(
                    {
                        ...
                        "valid_artifact": str(valid),
                        "invalid_artifact": str(invalid),
                        "quick_validate_artifact": str(quick_validate_artifact),
                        "next_slice_candidate": "static_dependency_overlay_contract",
                    }
                ),
                encoding="utf-8",
            ...
            bridge_payload = json.loads(bridge_json.read_text(encoding="utf-8"))
            normalized_payload = json.loads(normalized_bundle.read_text(encoding="utf-8"))
            self.assertEqual(bridge.returncode, 0)
            self.assertEqual(bridge_payload["status"], "ok")
            self.assertEqual(bridge_payload["quick_validate_status"], "passed")
            self.assertEqual(normalized_payload["quick_validate_status"], "passed")
            self.assertIn("quick_validate artifact bridge", bridge_md.read_text(encoding="utf-8"))
            ...
            self.assertEqual(evaluation_payload["workflow_status"], "ready_for_next_slice")

    def test_bridge_quick_validate_artifact_invalid_when_status_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            contract = tmp / "contract.json"
            valid = tmp / "valid.json"
            ...
            invalid.write_text(json.dumps(INVALID_ARTIFACT), encoding="utf-8")
            quick_validate_artifact.write_text(
                json.dumps({"contract_family": "quick_validate_capture"}),
                encoding="utf-8",
            )
            bundle_draft.write_text(
                json.dumps(
            ...
            self.assertIn("quick_validate_artifact.status must be one of", payload["errors"][0])

    def test_bridge_captured_smoke_to_bundle_and_evaluate(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            contract = tmp / "contract.json"
            valid = tmp / "valid.json"
            invalid = tmp / "invalid.json"
            smoke_capture = tmp / "smoke-capture.json"
            bundle_draft = tmp / "bundle-draft.json"
            bridge_json = tmp / "bridge.json"
            bridge_md = tmp / "bridge.md"
            normalized_bundle = tmp / "bundle.json"
            ...
            valid.write_text(json.dumps(VALID_ARTIFACT), encoding="utf-8")
            invalid.write_text(json.dumps(INVALID_ARTIFACT), encoding="utf-8")
            smoke_capture.write_text(
                json.dumps(
                    {
                        "status": "valid",
                        "generated_at": "2026-03-19T01:07:00+09:00",
                ...
                json.dumps(
                    {
                        ...
                        "invalid_artifact": str(invalid),
                        "quick_validate_status": "passed",
                        "smoke_capture_artifact": str(smoke_capture),
                        "next_slice_candidate": "static_dependency_overlay_contract",
                    }
                ),
                encoding="utf-8",
            ...
            self.assertEqual(bridge.returncode, 0)
            self.assertEqual(bridge_payload["status"], "ok")
            self.assertEqual(bridge_payload["smoke_capture_status"], "valid")
            self.assertEqual(normalized_payload["smoke_capture_status"], "valid")
            self.assertIn("captured smoke bridge", bridge_md.read_text(encoding="utf-8"))
    ...
    def test_bridge_captured_smoke_to_bundle_invalid_when_artifact_missing_status(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            contract = tmp / "contract.json"
            valid = tmp / "valid.json"
            ...
            self.assertEqual(result.returncode, 1)
            self.assertEqual(payload["status"], "invalid")
            self.assertIn("smoke_capture_artifact.status must be one of", payload["errors"][0])
