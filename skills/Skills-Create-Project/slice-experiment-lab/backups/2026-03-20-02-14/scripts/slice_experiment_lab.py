...
Usage:
    ...
    python3 slice_experiment_lab.py bridge-quick-validate-artifact --input-bundle-draft <file>
...
def emit_experiment_bundle_contract() -> dict[str, object]:
    return {
        ...
        "field_schema": {
            ...
            "quick_validate_artifact": "optional[path[str]]",
            "smoke_capture_artifact": "optional[path[str]]",
            "smoke_capture_status": "optional[enum(valid|invalid|capture_failed)]",
...
def bridge_quick_validate_artifact(input_path: Path, payload: dict[str, object]) -> dict[str, object]:
    errors: list[str] = []
    required_draft_fields = [
        ...
        "valid_artifact",
        "invalid_artifact",
        "quick_validate_artifact",
        "next_slice_candidate",
    ]
    for field in required_draft_fields:
        if field not in payload:
            errors.append(f"missing field: {field}")
        elif not isinstance(payload.get(field), str) or not str(payload.get(field)).strip():
            errors.append(f"{field} must be non-empty str")

    normalized_bundle: dict[str, object] | None = None
    quick_validate_status: str | None = None
    quick_validate_artifact = payload.get("quick_validate_artifact")
    if isinstance(quick_validate_artifact, str) and quick_validate_artifact.strip():
        artifact_path = Path(quick_validate_artifact)
        if not artifact_path.is_file():
            errors.append(f"quick_validate_artifact file does not exist: {quick_validate_artifact}")
        else:
            try:
                artifact = _load_json(artifact_path)
            except Exception as exc:  # noqa: BLE001
                errors.append(f"quick_validate_artifact cannot be loaded as dict JSON: {exc}")
            else:
                family = artifact.get("contract_family")
                if family != "quick_validate_capture":
                    errors.append("quick_validate_artifact.contract_family must be quick_validate_capture")
                raw_status = artifact.get("status")
                if not isinstance(raw_status, str) or raw_status not in QUICK_VALIDATE_ENUM:
                    errors.append(f"quick_validate_artifact.status must be one of {QUICK_VALIDATE_ENUM}")
                else:
                    quick_validate_status = raw_status

    if not errors and quick_validate_status is not None:
        normalized_bundle = {
            "skill_name": payload["skill_name"],
            "current_slice": payload["current_slice"],
            "contract_artifact": payload["contract_artifact"],
            "valid_artifact": payload["valid_artifact"],
            "invalid_artifact": payload["invalid_artifact"],
            "quick_validate_status": quick_validate_status,
            "quick_validate_artifact": payload["quick_validate_artifact"],
            "next_slice_candidate": payload["next_slice_candidate"],
        }
        notes = payload.get("notes")
        if isinstance(notes, str) and notes.strip():
            normalized_bundle["notes"] = notes

    return {
        "status": "ok" if not errors else "invalid",
        "generated_at": _now_iso(),
        "contract_family": "quick_validate_artifact_bundle_bridge",
        "input_bundle_draft": _relative_or_str(input_path),
        "quick_validate_artifact": quick_validate_artifact if isinstance(quick_validate_artifact, str) else None,
        "quick_validate_status": quick_validate_status,
        "errors": errors,
...
def bridge_captured_smoke_to_bundle(input_path: Path, payload: dict[str, object]) -> dict[str, object]:
    errors: list[str] = []
    required_draft_fields = [
        "skill_name",
        "current_slice",
    ...
    smoke_capture_status: str | None = None
    smoke_capture_artifact = payload.get("smoke_capture_artifact")
    if isinstance(smoke_capture_artifact, str) and smoke_capture_artifact.strip():
        artifact_path = Path(smoke_capture_artifact)
        if not artifact_path.is_file():
            errors.append(f"smoke_capture_artifact file does not exist: {smoke_capture_artifact}")
        else:
            try:
                artifact = _load_json(artifact_path)
            except Exception as exc:  # noqa: BLE001
                errors.append(f"smoke_capture_artifact cannot be loaded as dict JSON: {exc}")
            else:
                family = artifact.get("contract_family")
                if family != "smoke_command_capture":
                    errors.append("smoke_capture_artifact.contract_family must be smoke_command_capture")
                raw_status = artifact.get("status")
                if not isinstance(raw_status, str) or raw_status not in {"valid", "invalid", "capture_failed"}:
                    errors.append("smoke_capture_artifact.status must be one of ['valid', 'invalid', 'capture_failed']")
                else:
                    smoke_capture_status = raw_status

    normalized_bundle: dict[str, object] | None = None
    if not errors and smoke_capture_status is not None:
        normalized_bundle = {
            "skill_name": payload["skill_name"],
            "current_slice": payload["current_slice"],
            "contract_artifact": payload["contract_artifact"],
            "valid_artifact": payload["valid_artifact"],
            "invalid_artifact": payload["invalid_artifact"],
            "quick_validate_status": payload["quick_validate_status"],
            "smoke_capture_artifact": payload["smoke_capture_artifact"],
            "smoke_capture_status": smoke_capture_status,
            "next_slice_candidate": payload["next_slice_candidate"],
        }
        notes = payload.get("notes")
        if isinstance(notes, str) and notes.strip():
    ...
    return {
        "status": "ok" if not errors else "invalid",
        "generated_at": _now_iso(),
        "contract_family": "captured_smoke_to_bundle_adapter",
        "input_bundle_draft": _relative_or_str(input_path),
        "smoke_capture_artifact": smoke_capture_artifact if isinstance(smoke_capture_artifact, str) else None,
        "smoke_capture_status": smoke_capture_status,
        "errors": errors,
        "normalized_bundle": normalized_bundle,
    }
...
def gate_strict_warning_policy(input_path: Path, payload: dict[str, object]) -> dict[str, object]:
    ...
    recommended_action = "advance_to_next_slice" if decision == "pass" else "fix_or_repeat_current_slice"
    return {
        "status": decision,
        "generated_at": _now_iso(),
...
def evaluate_experiment_bundle(input_path: Path, payload: dict[str, object]) -> dict[str, object]:
    ...
    for key in ["contract_artifact", "valid_artifact", "invalid_artifact"]:
        ...
        if not isinstance(raw_path, str) or not raw_path.strip():
            continue
        path = Path(raw_path)
        artifact_results[key] = _relative_or_str(path)
        if not path.is_file():
    ...
    recommended_next = payload.get("next_slice_candidate") if workflow_status == "ready_for_next_slice" else None
    recommended_action = "advance_to_next_slice" if workflow_status == "ready_for_next_slice" else "fix_or_repeat_current_slice"
...
def render_evaluation_markdown(payload: dict[str, object]) -> str:
    if payload["contract_family"] == "quick_validate_artifact_bundle_bridge":
        lines = [
            "# slice-experiment-lab quick_validate artifact bridge",
            "",
            f"- generated_at: `{payload['generated_at']}`",
            f"- input_bundle_draft: `{payload['input_bundle_draft']}`",
            f"- status: `{payload['status']}`",
            f"- quick_validate_artifact: `{payload['quick_validate_artifact']}`",
            f"- quick_validate_status: `{payload['quick_validate_status']}`",
            "",
            "## Errors",
            "",
        ]
        if payload["errors"]:
            ...
            lines.append("- none")
        normalized_bundle = payload.get("normalized_bundle")
        lines.extend(["", "## Normalized Bundle", ""])
        if isinstance(normalized_bundle, dict):
            for key in REQUIRED_FIELDS:
                lines.append(f"- `{key}`: `{normalized_bundle.get(key)}`")
        else:
            lines.append("- none")
        return "\n".join(lines) + "\n"
    if payload["contract_family"] == "captured_smoke_to_bundle_adapter":
        lines = [
            "# slice-experiment-lab captured smoke bridge",
            "",
            f"- generated_at: `{payload['generated_at']}`",
            f"- input_bundle_draft: `{payload['input_bundle_draft']}`",
            f"- status: `{payload['status']}`",
            f"- smoke_capture_artifact: `{payload['smoke_capture_artifact']}`",
            f"- smoke_capture_status: `{payload['smoke_capture_status']}`",
            "",
            "## Errors",
            "",
        ]
        ...
        lines.extend(["", "## Normalized Bundle", ""])
        if isinstance(normalized_bundle, dict):
            for key in REQUIRED_FIELDS + ["smoke_capture_artifact", "smoke_capture_status"]:
                lines.append(f"- `{key}`: `{normalized_bundle.get(key)}`")
        else:
            lines.append("- none")
        return "\n".join(lines) + "\n"
    ...
    lines = [
        ...
        f"- recommended_next_slice: `{payload['recommended_next_slice']}`",
        "",
        "## Errors",
        "",
    ]
...
def cmd_bridge_quick_validate_artifact(args: argparse.Namespace) -> int:
    input_path = Path(args.input_bundle_draft)
    try:
        payload = _load_json(input_path)
        bridge_payload = bridge_quick_validate_artifact(input_path, payload)
    except Exception as exc:  # noqa: BLE001
        print(f"[ERROR] {exc}", file=sys.stderr)
        return 1
    if args.output_json:
        _write_json(args.output_json, bridge_payload)
    else:
        print(json.dumps(bridge_payload, indent=2, ensure_ascii=False))
    if args.output_md:
        _write_md(args.output_md, render_evaluation_markdown(bridge_payload))
    if args.output_bundle and isinstance(bridge_payload.get("normalized_bundle"), dict):
        _write_json(args.output_bundle, bridge_payload["normalized_bundle"])
    return 0 if bridge_payload["status"] == "ok" else 1


def cmd_bridge_captured_smoke_to_bundle(args: argparse.Namespace) -> int:
    input_path = Path(args.input_bundle_draft)
    try:
        payload = _load_json(input_path)
        bridge_payload = bridge_captured_smoke_to_bundle(input_path, payload)
    except Exception as exc:  # noqa: BLE001
        print(f"[ERROR] {exc}", file=sys.stderr)
        return 1
    if args.output_json:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    eval_parser = subparsers.add_parser(
        ...
        help="Evaluate contract/valid/invalid triad plus quick_validate status and recommend next slice action.",
    )
    ...
    bridge_parser = subparsers.add_parser(
        ...
        help="Normalize a quick_validate capture artifact into canonical experiment bundle input.",
    )
    bridge_parser.add_argument("--input-bundle-draft", required=True, help="Path to bundle draft with quick_validate_artifact.")
    bridge_parser.add_argument("--output-json", help="Optional output path for bridge report JSON.")
    bridge_parser.add_argument("--output-md", help="Optional output path for bridge markdown summary.")
    bridge_parser.add_argument("--output-bundle", help="Optional output path for normalized canonical bundle JSON.")
    bridge_parser.set_defaults(func=cmd_bridge_quick_validate_artifact)

    smoke_bridge_parser = subparsers.add_parser(
        "bridge-captured-smoke-to-bundle",
        help="Normalize a smoke capture artifact into canonical experiment bundle input.",
    )
    smoke_bridge_parser.add_argument("--input-bundle-draft", required=True, help="Path to bundle draft with smoke_capture_artifact.")
    smoke_bridge_parser.add_argument("--output-json", help="Optional output path for bridge report JSON.")
    smoke_bridge_parser.add_argument("--output-md", help="Optional output path for bridge markdown summary.")
    smoke_bridge_parser.add_argument("--output-bundle", help="Optional output path for normalized canonical bundle JSON.")
    smoke_bridge_parser.set_defaults(func=cmd_bridge_captured_smoke_to_bundle)
