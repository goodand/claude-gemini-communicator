...
- 현재 slice를 닫고 다음 slice로 넘어갈지 판단한다
...
- `next_slice_candidate`가 비어 있지 않다
...
- `현재 contract slice를 닫고 다음 slice로 넘어갈지` 판단은 skill 내부 실험 루프로 분리하는 편이 낫다.
- `contract / valid / invalid` triad와 `quick_validate_status`는 함께 읽어야 다음 조각 판단을 안정적으로 할 수 있다.
- 이 skill은 smoke runner가 아니라 experiment evaluator여야 한다.
- 다음 slice 후보는 공용 목록이 아니라 대상 skill의 implementation checklist에서 온다.

## Current Implementation Target

v0.2는 `experiment_bundle_contract`, `evaluate-experiment-bundle`, `artifact_triad_naming_helper`, `quick_validate_capture_adapter`, `smoke_command_capture_adapter`까지 구현한다.
